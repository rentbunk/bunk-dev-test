...
export class TravelService {
  ...
  editExpense(newExpense: Expense) {
    ...
    this.expenseList.next(currentExpenses);
    this.selectedExpenseIndex.next(null); // Just once is enough
  }
  ...
  setEditData(index: number) {
    ...
    // Reset any other item which might be in EDITING or DELETING state
    if (selectedIndex !== null)
      currentExpenses[selectedIndex].status = ExpenseStatus.ACTIVE;
  ...
  setRemoveData(index: number) {
    ...
    // Set the clicked item to DELETING state
